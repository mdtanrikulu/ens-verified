...
contract ECDSAProofVerifier is IProofVerifier {
    ...
    function verifyProof(bytes calldata proof, bytes32 recordDataHash, address issuer) external pure returns (bool) {
        address signer = ECDSA.recover(MessageHashUtils.toEthSignedMessageHash(recordDataHash), proof);
